...
def achievement_list():
    ...
    # Poprawka 1
    for achievement in achievements_by_category.copy():
        if len(achievements_by_category[achievement]) == 0:
            del achievements_by_category[achievement]
...
class AchievementSendToFixSingleForm(FlaskForm):
    to_fix_comment = StringField('Komentarz dla użytkownika', validators=[DataRequired(), Length(min=1, max=200)])
    submit = SubmitField("Wyślij do poprawy")

@achievement.route('/achievement_send_to_fix_single_form/<int:aid>', methods=['GET', 'POST'])
@login_required
@admin_required
def achievement_send_to_fix_single_form(aid):
    form = AchievementSendToFixSingleForm()
    ...
    if form.validate_on_submit():
        try:
            current_achieviement.to_fix_comment = form.to_fix_comment.data
            current_achieviement.status = AchievementState.TO_BE_FIXED.value
            db.session.commit()

            flash_bootstrap_success(f'Wysłano osiągnięcie do poprawy')
            return redirect(url_for('achievement.achievement_rate'))
        except IntegrityError as e:
            db.session.rollback()
            flash_bootstrap_danger(e)
        ...
        if current_achieviement.to_fix_comment == None:
            current_achieviement.to_fix_comment = ""

    return render_template('achievement_send_to_fix_single_form.html', achievement_fields=current_achieviement, form=form)
...
class AchievementFixSingleForm(FlaskForm):
    ...
    category = SelectField('Kategoria', coerce=int)
    hashtag = SelectField('Hashtag', coerce=int)
    to_fix_comment = StringField('Komentarz dla użytkownika')
    ...
    def __init__(self, *args, obj=None, **kwargs):
        ...
        if obj:
            ...
            self.to_fix_comment.data = obj.to_fix_comment
...
def achievement_fix_single_form(aid):
    ...
    if form.validate_on_submit():
        try:
            ...
            current_achieviement.rating = None # Poprawka 2
            current_achieviement.to_fix_comment = None
